...
```
```{r}
boxplot(Stress_level, col="green", xlab="Stress Level", ylab = " frequency", main = "Distribution of Stress Level")
```

## Sleep Duration
```{r}
Sleep_Duration <- sleep$Sleep.Duration
mean(Sleep_Duration) 
median(Sleep_Duration) 
sd(Sleep_Duration)
IQR(Sleep_Duration)

```
```{r}
boxplot(Sleep_Duration, col="blue", xlab="Sleep Duration", ylab = " frequency", main = "Distribution of Sleep Duration")
```
...
fviz_nbclust(df_sleep, hcut, method = "silhouette")+
  labs(subtitle = "Silhouette method")
```
Interpretation:The silhouette analysis for hierarchical clustering shows a steady increase in silhouette width with the number of clusters, peaking at 9 clusters, which may suggest that dividing the data into 9 clusters achieves the best structure and separation.
...
ggplot(variables, aes(x = Heart.Rate, y = Daily.Steps, color = factor(cluster_sleep))) +
  ...
  theme_minimal()
```
Interpretation: The scatter plot shows the distribution of individuals into six clusters based on their heart rate and daily steps, with clear differentiation among clusters suggesting distinct groupings based on these two health and activity metrics.
...
```{r}
pam.res <- pam(scaled_sleep, 5)
fviz_cluster(pam.res, 
             palette = c("red", "orange","yellow","green","blue"), # color palette
             ellipse.type = "t", # Concentration ellipse
             repel = TRUE, # Avoid label overplotting (slow)
             ggtheme = theme_classic()
             )
```
Interpretation: We can see that there are 5 clusters, each cluster may have a specific health implication such individuals with sedentary lifestyle, active individuals with good cardiovascular health, individuals with moderate phyiscal activity, individuals with lower cardiovascular demands, and individuals that are active with potential stress affecting heart rate.
